...
function tab(item, type) {
    ...
    if (type !== "tab") {
		...
		document.getElementById('editor-buttons').classList = 'editor-buttons';
		
		
		
		// highlight the tab menu button we have activated
		if(item == "Objects" || item == "Menu" || item == "Editor" || item == "Timebar" || item == "Headbands" || item == "Effects"){
		   document.getElementById('editor-buttons').classList = 'editor-buttons active'; 
		}
		else {
		    var b = document.getElementById('b'+item);
		    b.classList = 'tab active';
		    
		    document.getElementById("editor-buttons").selectedIndex = 0;
		}
